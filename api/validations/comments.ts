...
export const commentsIdParamSchema = z.object({
  id: z.string(),
});
...
// Schema for comments search query parameters
export const commentsSearchQuerySchema = z.object({
  query: z
    .string({
...
interface ProcessedCommentsQueryParams {
  where: Record<string, any>; // Using Record<string, any> for the dynamic where clause
  skip: number;
  take: number;
}

export const processCommentsQueryParams = (
  query: z.infer<typeof commentsQuerySchema>,
): ProcessedCommentsQueryParams => {
  ...
  if (!parseResult.success) {
    ...
    console.error(
      'comments query parameter processing error:',
      parseResult.error,
    );
