...
export const hashtagsIdParamSchema = z.object({
  id: z.string(),
});

// Schema for query parameters (expecting strings from URL)
export const hashtagsQuerySchema = z.object({
  page: z
    .string()
    .optional()
...
// Schema for hashtags search query parameters
export const hashtagsSearchQuerySchema = z.object({
  query: z
    .string({
...
export const updateHashtagsSchema = z.object({
  name: z.string().min(1, { message: 'Name is required and cannot be empty' }),
});
...
interface ProcessedHashtagsQueryParams {
  where: Record<string, any>; // Using Record<string, any> for the dynamic where clause
  skip: number;
  take: number;
}

export const processHashtagsQueryParams = (
  query: z.infer<typeof hashtagsQuerySchema>,
): ProcessedHashtagsQueryParams => {
  ...
  if (!parseResult.success) {
    ...
    console.error(
      'hashtags query parameter processing error:',
      parseResult.error,
    );
