...
// Schema for posts search query parameters
export const postsSearchQuerySchema = z.object({
  query: z
    .string({
...
interface ProcessedPostsQueryParams {
  where: Record<string, any>; // Using Record<string, any> for the dynamic where clause
  skip: number;
  take: number;
}

export const processPostsQueryParams = (
  query: z.infer<typeof postsQuerySchema>,
): ProcessedPostsQueryParams => {
  ...
  if (!parseResult.success) {
    ...
    console.error('posts query parameter processing error:', parseResult.error);
