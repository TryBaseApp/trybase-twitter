...
import {
  ...
  updateFollowersSchema,
  followersIdParamSchema,
  followersQuerySchema,
  followersSearchQuerySchema,
  processFollowersQueryParams,
} from '../validations/followers';

const app = new Hono()
  // GET /followers - Get all followers
  .get('/', zValidator('query', followersQuerySchema), async c => {
    try {
      const validatedQuery = c.req.valid('query');
      const options = processFollowersQueryParams(validatedQuery);
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.followers.count({ where: whereClause }),
      ]);
      ...
      console.error('Failed to retrieve followers:', error); // Log error
      c.status(500);
      return c.json({
  ...
  // GET /followers/:id - Get Followers by ID
  .get('/:id', zValidator('param', followersIdParamSchema), async c => {
    try {
      ...
      const item = await prisma.followers.findUnique({
        where: whereClause,
      ...
      if (!item) {
        c.status(404);
        return c.json({ error: 'followers not found' });
      }
  ...
  // POST /followers - Create a new Followers
  .post('/', zValidator('json', createFollowersSchema), async c => {
    try {
      // Get validated data from the request body middleware (with cast)
      const validatedData = c.req.valid('json');

      // Create followers using the validated data
      const item = await prisma.followers.create({
        data: validatedData,
        // https://www.prisma.io/docs/orm/reference/prisma-client-reference#include
        // include: { name_of_relation: true }
      });
      ...
      console.error('Failed to create followers:', error);
      ...
      if (error?.code === 'P2002') {
        ...
        return c.json({
          error: 'Could not create followers',
          // @ts-expect-error: error is unknown
  ...
  // PUT /followers/:id - Update Followers by ID
  .put(
    '/:id',
    zValidator('param', followersIdParamSchema),
    zValidator('json', updateFollowersSchema),
    async c => {
      try {
        ...
        const item = await prisma.followers.update({
          where: whereClause,
          data: validatedData,
          // https://www.prisma.io/docs/orm/reference/prisma-client-reference#include
          // include: { name_of_relation: true }
        ...
        console.error(
          `Failed to update followers ${c.req.param('id')}:`,
          error,
        );
  ...
  // DELETE /followers/:id - Delete Followers by ID
  .delete('/:id', zValidator('param', followersIdParamSchema), async c => {
    try {
      ...
      return c.json({
        success: true,
        message: 'followers deleted successfully',
      });
    } catch (error: unknown) {
      console.error(`Failed to delete followers ${c.req.param('id')}:`, error);
      // Use the service function to handle 'not found' (P2025)
      return handleNotFound(error, c);
    }
  })
  // GET /followers/search - Search followers by name
  .get('/search', zValidator('query', followersSearchQuerySchema), async c => {
    try {
      // Get validated query parameters from middleware
      const validatedQuery = c.req.valid('query');

      // Pagination logic (similar to processfollowersQueryParams but simpler for just page/limit)
      ...
      c.status(500);
      return c.json({
        error: 'Failed to search followers',
        details:
          error instanceof Error ? error.message : 'Unknown internal error',
      });
    }
