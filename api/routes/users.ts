...
const app = new Hono()
  ...
  .get('/', zValidator('query', usersQuerySchema), async c => {
    try {
      ...
      const options = processUsersQueryParams(validatedQuery);
      ...
      console.error('Failed to retrieve users:', error); // Log error
      c.status(500);
      return c.json({
  ...
  .get('/:id', zValidator('param', usersIdParamSchema), async c => {
    try {
  ...
  .post('/', zValidator('json', createUsersSchema), async c => {
    try {
      ...
      // Create users using the validated data
  ...
  .put(
    '/:id',
    zValidator('param', usersIdParamSchema),
    ...
    async c => {
      try {
        ...
        console.error(`Failed to update users ${c.req.param('id')}:`, error);
  ...
  .delete('/:id', zValidator('param', usersIdParamSchema), async c => {
    try {
      ...
      console.error(`Failed to delete users ${c.req.param('id')}:`, error);
      // Use the service function to handle 'not found' (P2025)
      return handleNotFound(error, c);
    }
  })
  // GET /users/search - Search users by name
  .get('/search', zValidator('query', usersSearchQuerySchema), async c => {
    try {
      // Get validated query parameters from middleware
      const validatedQuery = c.req.valid('query');

      // Pagination logic (similar to processusersQueryParams but simpler for just page/limit)
